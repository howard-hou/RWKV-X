...
def generate_random_name():
    """randomly generate a 3-letter uppercase name"""
    return ''.join(random.choices(string.ascii_uppercase, k=3))

def generate_random_phone():
    """randomly generate a 6-digit phone number"""
    return ''.join(random.choices(string.digits, k=6))

def generate_phonebook_list(num_pairs):
    """generate a list of (Name, Phone) pairs of specified length, ensuring unique names"""
    data = []
    existing_names = set()

    while len(data) < num_pairs:
        nm = generate_random_name()
        if nm not in existing_names:
            existing_names.add(nm)
            ph = generate_random_phone()
            data.append({"name": nm, "phone": ph})
    return data

def build_context_string(data):
    context = ""
    for item in data:
        context += f"Name: {item['name']}, Phone: {item['phone']}\n"
    return context
# =========================
# load dataset
# =========================
...
prefix_context = build_context_string(generate_phonebook_list(20)) # 20 pairs -> 20 lines -> 120 tokens
prefix_tokens = tokenizer.encode(prefix_context)
suffix_context = build_context_string(generate_phonebook_list(4000)) # 4000 pairs -> 4000 lines -> 24000 tokens
suffix_tokens = tokenizer.encode(suffix_context)
